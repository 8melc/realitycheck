...
const COOKIE_NAME = 'realitycheck-life-data';
...
export default function LifeWeeksComponent({ onClose, isInline = false }: LifeWeeksComponentProps) {
  ...
  if (showBlackhole) {
    return (
      <div className={`${isInline ? 'lifeweeks-inline' : 'min-h-screen'} bg-realitycheck-noir flex items-center justify-center`}>
  ...
  if (!isCalculated) {
    return (
      <div className={`${isInline ? 'lifeweeks-inline' : 'min-h-screen'} bg-realitycheck-noir flex items-center justify-center px-6`}>
        <div className="max-w-md w-full">
          <div className="text-center mb-8">
            <h1 className="font-display text-4xl font-bold text-realitycheck-cream mb-4">
              Deine Lebenswochen
            </h1>
            <p className="text-realitycheck-steel">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label htmlFor="birthdate" className="block text-sm font-medium text-realitycheck-cream mb-2">
                Geburtsdatum
              </label>
              <input
                type="date"
                ...
                className="w-full px-4 py-3 bg-realitycheck-charcoal border border-realitycheck-smoke rounded-lg text-realitycheck-cream focus:border-realitycheck-coral focus:outline-none"
                required
              />
            </div>

            <div>
              <label htmlFor="targetAge" className="block text-sm font-medium text-realitycheck-cream mb-2">
                Zielalter (optional)
              </label>
              <input
                type="number"
            ...
            <button
              type="submit"
              className="w-full bg-realitycheck-coral hover:bg-realitycheck-coral-dark text-white font-semibold py-3 px-6 rounded-lg transition-colors"
            >
  ...
  return (
    <div className={`${isInline ? 'lifeweeks-inline' : 'min-h-screen'} bg-realitycheck-noir text-realitycheck-cream`}>
      {/* Header */}
      <div className="relative">
        <div className="absolute inset-0 bg-gradient-to-b from-realitycheck-noir via-realitycheck-charcoal to-realitycheck-noir"></div>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="flex justify-between items-start mb-8">
              <div>
                ...
                <p className="text-realitycheck-steel text-lg max-w-2xl">
              ...
              {onClose && (
                <button
                  onClick={handleClose}
                  className="text-realitycheck-steel hover:text-realitycheck-coral transition-colors p-2"
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 bg-realitycheck-coral rounded"></div>
                <span className="text-realitycheck-steel text-sm">Vergangenheit</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 bg-realitycheck-mint rounded"></div>
                <span className="text-realitycheck-steel text-sm">Gegenwart</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 bg-realitycheck-charcoal border border-realitycheck-smoke rounded"></div>
                <span className="text-realitycheck-steel text-sm">Zukunft</span>
              </div>
              
              <button
                ref={legendTriggerRef}
                onClick={() => setIsLegendOpen(!isLegendOpen)}
                className="text-realitycheck-mint hover:text-realitycheck-mint-dark text-sm underline"
              >
                Mehr erfahren
              </button>
            </div>
            ...
            {isLegendOpen && (
              <div
                ...
                className="absolute z-20 bg-realitycheck-charcoal border border-realitycheck-smoke rounded-lg p-4 max-w-md"
              >
                <h3 className="font-semibold text-realitycheck-cream mb-2">Was bedeuten die Farben?</h3>
                <p className="text-realitycheck-steel text-sm mb-2">
                  <span className="text-realitycheck-coral">Rote Wochen</span> zeigen deine Vergangenheit – Zeit, die du bereits gelebt hast.
                </p>
                <p className="text-realitycheck-steel text-sm mb-2">
                  <span className="text-realitycheck-mint">Grüne Wochen</span> markieren deine Gegenwart – die Zeit, in der du jetzt lebst.
                </p>
                <p className="text-realitycheck-steel text-sm">
                  <span className="text-realitycheck-steel">Graue Wochen</span> repräsentieren deine Zukunft – Zeit, die noch vor dir liegt.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="px-6 pb-12">
          <div className="max-w-6xl mx-auto">
                ...
                return (
                  <div
                    ...
                    className={`w-2 h-2 rounded-sm ${
                      isPast 
                        ? 'bg-realitycheck-coral' 
                        : isCurrent 
                        ? 'bg-realitycheck-mint' 
                        : 'bg-realitycheck-charcoal border border-realitycheck-smoke'
            ...
            <div className="space-y-8">
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${
                        activeTab === tab.id
                          ? 'bg-realitycheck-coral text-white'
                          : 'bg-realitycheck-charcoal text-realitycheck-steel hover:bg-realitycheck-smoke'
                      }`}
                    >
                      <IconComponent />
                      {tab.label}
              ...
              <div className="bg-realitycheck-charcoal rounded-xl p-6">
                <h2 className="font-display text-2xl font-bold mb-6 text-realitycheck-cream">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {statsTabs[activeTab].cards.map((card) => (
                    <div key={card.id} className="bg-realitycheck-noir p-4 rounded-lg border border-realitycheck-smoke">
                      <h3 className="font-semibold text-realitycheck-coral mb-2">{card.title}</h3>
                      <div className="text-2xl font-bold text-realitycheck-cream mb-2">
                        {card.metric(stats)}
                      </div>
                      <p className="text-realitycheck-steel text-sm">{card.body}</p>
                    </div>
                  ))}
                </div>
              </div>
